...
final_modeling <- function(ready_final_model, toi = NULL, analysis_type) {
  ...
  if (analysis_type == "Exploratory") {
    ...
    tab1 <- map_dfr(.x = output_tables, .f = ~ {
      .x$tab1 %>% dplyr::filter(!grepl("Average", `Times Included`))
    })
    tab2 <- map_dfr(.x = output_tables, .f = ~ {
      .x$tab2 %>% dplyr::filter(!grepl("Average", `Times Included`))
    })
    tab3 <- map_dfr(.x = output_tables, .f = ~ {
      .x$tab3 %>% dplyr::filter(!grepl("Average", `Times Included`))
    })
