...
prism_plot <- function(data, tables, trt_sel,
                       ...
                       inputs = NULL, same_ylim = FALSE) {{{ box::use(ggprism)
  box::use(ggplot2)
  box::use(dplyr)
...
p_vals <- dplyr$select(p_vals, Treatment, `Time Points`, grep("p value from", colnames(.)))
p_vals <- dplyr$mutate_at(p_vals, .vars = 3:ncol(.), .funs = ~ as.character(.))
