
#' @export
ui_app <- function(id = "app") {
  box::use(shiny)
  box::use(. / router)
  box::use(. / test_theme)
  box::use(bs4Dash)
  box::use(. / navbar)
  box::use(. / footer)
  ns <- shiny$NS(id)
  router <- router$router(ns)

  header <- bs4Dash$dashboardHeader(
    shiny$tags$div(
      test_theme$headers(),
      navbar$ui_navbar(ns("navbar"))
    )
  )

  body <- bs4Dash$dashboardBody(
    router$ui
  )

  footer <-
    bs4Dash$dashboardFooter(
      footer$ui_footer(ns("footer"))
    )

  page <- bs4Dash$dashboardPage(
    fullscreen = FALSE, dark = FALSE,
    header = header,
    body = body,
    sidebar = bs4Dash$dashboardSidebar(disable = T),
    footer = footer
  )

  shiny$tags$div(class = "bg-light", page)
}

#' @export
server_app <- function(id = "app") {
  box::use(shiny)
  box::use(. / router)
  box::use(. / footer)
  box::use(. / ui_home)
  box::use(. / navbar)
  box::use(. / download_historical)
  box::use(. / analysis_a_run)
  box::use(. / analysis_a_session_setup)
  box::use(future)
  # library(test)
  shiny$moduleServer(
    id,
    function(input, output, session) {
      future$plan(future$multiprocess)
      ns <- session$ns
      router <- router$router(ns)
      router$server(input, output, session)
      footer$server_footer()
      ui_home$server_home("home")
      navbar$server_navbar("navbar")
      download_historical$server_download_historical("download_historical")
      session_out <- analysis_a_session_setup$test_session_setup_server("test_session_setup")
      analysis_a_run$analysis_a_run_server("analysis_a_run", session_out, getOption("cachetest", FALSE))
    }
  )
}
