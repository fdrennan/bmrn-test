...
analysis_a_run <- function(id = "analysis_a", user, is_admin) {
  {
    box::use(shiny)
    box::use(glue)
    box::use(. / analysis/one/analysis_a_setup)
    box::use(. / analysis/one/analysis_a_report)
    box::use(. / app/testSpinner)
    box::use(. / graphics/prism)
  }
  ns <- shiny$NS(id)
  shiny$tabsetPanel(
    id = ns("test_1_tabs"),
...
analysis_a_run_server <- function(id, input_signal, cache = FALSE) {
  {
    ...
    box::use(storr)
    box::use(fs)
    box::use(../.. / database/connect_table)
    box::use(. / app/testSpinner)
    box::use(. / analysis/one/analysis_a_setup)
    box::use(. / graphics/vizualization)
    ...
    function(input, output, session) {
      ...
      output$analysisInputsData <- shiny$renderUI({
        ...
        if (analysis_type == "Exploratory") {
          ...
          tables$tab0 <- dplyr$filter(tables$tab0, `Time Points` %in% times)
          tables$tab1 <- dplyr$filter(tables$tab1, `Time Points` %in% times)
          tables$tab2 <- dplyr$filter(tables$tab2, `Time Points` %in% times)
          tables$tab3 <- dplyr$filter(tables$tab3, `Time Points` %in% times)
        }
