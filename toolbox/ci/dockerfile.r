...
#' @export
install_system_requirements <- function(d, libraries) {
  box::use(glue[glue])
...
#' @export
install_r <- function(d, install_java = TRUE) {
  ...
  dockerfile$install_system_requirements(
    d, c(
      "tzdata", "wget", "vim", "curl", "git-core", "curl", "lsb-release", "libssl-dev", "libgit2-dev", "libcurl4-openssl-dev",
      "libssh2-1-dev", "libsodium-dev", "libxml2-dev", "dirmngr", "libudunits2-dev", "libharfbuzz-dev", "libfribidi-dev",
      "zlib1g-dev", "libpq-dev", "libsasl2-dev", "cmake", "libfontconfig1-dev",
  ...
  dockerfile$install_system_requirements(d, c("r-base", "r-base-dev"))
  ...
  d$RUN(docker$r(install.packages("tidyverse", dependencies = TRUE)))

  if (install_java) {
    dockerfile$install_system_requirements(d, c(" default-jre", " default-jdk"))
    d$RUN("R CMD javareconf")
  }
}
...
dockerfile_build <- function(from = "ubuntu:jammy",
  ...
  if (copy_all) {
    cli$cli_alert_warning(
      "Storing everything in app dir in container",
      "This is generally unqise and may uncache your Docker build unnecessarily."
    )
    d$COPY(".", workdir)
  } else {
    if(!is.null(files)) {
  ...
  if (!is.null(expose)) {
    expose_txt <- paste0(expose, collapse = ", ")
    cli$cli_alert_info("Exposing ports {expose_txt}")
    purrr$walk(expose, function(x) d$EXPOSE(x))
  }

  dockerfile_path <- file.path(app_dir, "Dockerfile")
  readr$write_file(paste0(d$Dockerfile, collapse = "\n"), dockerfile_path)

  cli$cli_alert_success("{crayon::red} Dockerfile stored at cd {app_dir}")
}
