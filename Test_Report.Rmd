...
trans_name <- transform_table() %>%
  dplyr$filter(power == data$plot$data$box_cox) %>%
...
groups_diff <- dplyr$distinct(data$pre_modeling_input$transformed_data, Treatment, diff_group) %>%
  dplyr$filter(diff_group != "Pooled") %>%
...
data_check <- data$plot$data %>% cbind.data.frame(
...
session_info <- data$input_data$session_data %>%
  ...
  dplyr$mutate(
    ...
    col1 = dplyr$case_when(
      col1 == "statistician" ~ "Contact Statistician",
  ...
  dplyr$filter(!(col1 %in% c("Uuid", "Base Dir", "Rel Path Home", "Full Path Files", "Studytitle")))
...
contrasts <- data.frame(
  ...
  dplyr$mutate(Comparison = paste(Group_1, "vs.", Group_2))

contrasts_display <- contrasts %>% dplyr$select(Label, Comparison, `Purpose of Comparison`, `Multiple Testing Procedure`)
...
variance_result <- dplyr$if_else(length(groups_diff) == 0, paste("All groups have similar variance, thus a common variance component is estimated for all groups"),
  paste(paste(groups_diff, collapse = ""), "had a different variance from the basic model, thus the variance", dplyr$if_else(length(groups_diff) > 1, "components", "component"), "of", paste(groups_diff, collapse = ""), "were estimated separately.")
)
```
...
if (trans_name != "No transformation") {
  ...
  assumptions <- ggarrange(plotlist = list(
    ggplot2$ggplot(data = data_check, ggplot2$aes(x = resid_orig)) +
      geom_histogram(color = "white") +
      theme_bw() +
      ggplot2$ggtitle(paste("Histogram of the Residuals", data$plot$endpoint)) +
      ggplot2$theme(plot.title =ggplot2$(size = 10)),
    ggplot2$ggplot(data = data_check, ggplot2$aes(x = resid_transformed)) +
      geom_histogram(color = "white") +
      theme_bw() +
      ggplot2$ggtitle(paste("Histogram of the Residuals from", new_trans, data$plot$endpoint)) +
      ggplot2$theme(plot.title =ggplot2$(size = 10)),
    ggplot2$ggplot(data = data_check, ggplot2$aes(sample = resid_orig)) +
      stat_qq() +
      stat_qq_line() +
      theme_bw() +
      ggplot2$ggtitle(paste("Q-Q plot from", data$plot$endpoint)) +
      xlab("Theoretical Quantiles") +
      ggplot2$ylab("Sample quantile") +
      ggplot2$theme(plot.title =ggplot2$(size = 10)),
    ggplot2$ggplot(data = data_check, ggplot2$aes(sample = resid_transformed)) +
      stat_qq() +
      stat_qq_line() +
      theme_bw() +
      ggplot2$ggtitle(paste("Q-Q plot from", new_trans, data$plot$endpoint)) +
      xlab("Theoretical Quantiles") +
      ggplot2$ylab("Sample quantile") +
      ggplot2$theme(plot.title =ggplot2$(size = 10))
  ))
...
prism <- prism_plot(
  data = transformed_data, tables = data$tables$tables,
...
transform <- data$tables$power != 1
show_text <- transform
footer <- dplyr$if_else(!show_text,
...
endpoint <- dplyr$if_else(!show_text,
  data$plot$endpoint,
...
tab0 <- data$tables$tables$tab0 %>% dplyr$filter(`Time Points` %in% time)
tab0_ft <- word_tables(tab0,
  include_summ_stat = F, summary_only = T, transform = transform,
...
```{r, echo = F, error=F, warning=FALSE}
tab1 <- data$tables$tables$tab1 %>% dplyr$filter(`Time Points` %in% time)

tab1_ft <- word_tables(
  data = tab1, include_summ_stat = T, summary_only = F, transform = transform,
...
```{r, echo = F, error=F, warning=FALSE}
tab2 <- data$tables$tables$tab2 %>% dplyr$filter(`Time Points` %in% time)

tab2_ft <- word_tables(tab2,
  include_summ_stat = T, summary_only = F, transform = transform,
  footer = footer, endpoint = data$plot$endpoint
) %>%
  width(x = ., width = 0.75)
...
```{r, echo = FALSE, error=F, warning=FALSE}
tab3 <- data$tables$tables$tab3 %>% dplyr$filter(`Time Points` %in% time)
# debug(word_tables)
tab3_ft <- word_tables(tab3,
  include_summ_stat = T, summary_only = F, transform = transform,
  footer = footer, endpoint = data$plot$endpoint
) %>%
  width(x = ., width = 1)
