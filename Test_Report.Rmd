...
params:
  ...
  uuid: '9814f144-9e26-444d-ab50-055dab567549'
...
Test 1 can also address comparison of only two groups over multiple time points. The steps that check model assumptions and determinations an appropriate correlation structure remain the same. Howevery, multiple testing adjustments are not necessary.  
...
Note: If there are only two groups of interest, then there is no adjustment for multiple comparison. 
...
two_group = is.null(data$tables$tables$tab1)

```
`r if(!show_text & two_group) {"\\begin{comment}"}`

### `r ifelse(show_text & !two_group, paste('Table 1: Summary of', data$plot$endpoint),"")`

```{r conditional_block, eval=show_text&!two_group,echo=FALSE, error=F, warning=FALSE}
...
```
`r if(!show_text & two_group) {"\\end{comment}"}`

`r if(two_group) {"\\begin{comment}"}`
\newpage
### `r paste(ifelse(show_text & !two_group, 'Table 2: Comparison between Controls and Wild Type as to', 'Table 1 Comparison between Controls and Wild Type as to'),data$plot$endpoint)`

```{r, echo = F, error=F, warning=FALSE, eval = !two_group}
...
### `r paste(ifelse(show_text& !two_group, 'Table 3: Comparison among the Vehicle and Treatment Groups as to', 'Table 2: Comparison among the Vehicle and Treatment Groups as to'),data$plot$endpoint)`
...
### `r paste(ifelse(show_text& !two_group, 'Table 4: Comparison Doses and Controls/Wild Type as to', 'Table 3: Comparison Doses and Controls/Wild Type as to'),data$plot$endpoint)`

```{r, echo = FALSE, error=F, warning=FALSE, eval =!two_group }
...
`r if(two_group) {"\\end{comment}"}`

`r if(!two_group | !show_text) {"\\begin{comment}"}`

### `r ifelse(show_text & two_group, paste('Table 1: Summary of', data$plot$endpoint),"")`

```{r conditional_block_2, eval=show_text &two_group ,echo=FALSE, error=F, warning=FALSE}
tab0 = data$tables$tables$tab0 %>% filter(`Time Points` %in% time)
tab0_ft = word_tables(tab0, include_summ_stat = F, summary_only = T, transform = transform,
            footer = 'Group level summary on the original scale.', endpoint = data$plot$endpoint) %>%
  width(x = ., width = 1)
FitFlextableToPage(tab0_ft)
```
`r if(!two_group | !show_text) {"\\end{comment}"}`

`r if(!two_group) {"\\begin{comment}"}`
\newpage

### `r paste(ifelse(show_text & two_group, 'Table 2: Comparison among the Vehicle and Treatment Groups as to', 'Table 1: Comparison among the Vehicle and Treatment Groups as to'),data$plot$endpoint)`
