...
params:
  ...
  uuid: '5fd32f5a-454e-43df-9d5f-bf4fb5f02ba2'
...
baseline_selected = 'Baseline' %in% data$plot$ui_selections$time_sel


if ( data$plot$ui_selections$y_axis == "transform") {
      plots <- vizualization(
        transformed_data = pdata$transformed_data,
        power = pdata$box_cox,
        endpoint = data$plot$endpoint,
        baseline = FALSE, # logic is backwards in the functions
        transformation = TRUE,
        ui_sel = data$plot$ui_selections
      )
    }
    if ( data$plot$ui_selections$y_axis == "no_transform") {
      plots <- vizualization(
        transformed_data = pdata$transformed_data,
        power = pdata$box_cox,
        endpoint = data$plot$endpoint,
    ...
    if ( data$plot$ui_selections$y_axis == "change_from_baseline") {
      transformed_data <- pdata$transformed_data %>%
