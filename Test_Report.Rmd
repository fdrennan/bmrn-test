...
params:
  ...
  uuid: 'eb9137b5-9565-4c2c-b37b-df447940dfb9' 
...
time = if_else(analysis_type == 'Exploratory',prismInputs$timePlotSelectors, toi)
...
<!-- ```{r, echo = FALSE, error=F, warning=FALSE, fig.align='center',fig.cap=prism_caption_bar, out.width='200%'} -->
<!-- knitr::include_graphics(paste0(plots_path,'/prism_plots_bar.jpg')) -->
<!-- ``` -->

<!-- \newpage -->

<!-- ```{r, echo = FALSE, error=F, warning=FALSE,fig.align='center', fig.cap=prism_caption_box} -->
...
tab0 = data$tables$tables$tab0 %>% filter(`Time Points` %in% data$inputs$timeTreatmentSelectorsTable)
word_tables(tab0, include_summ_stat = F, summary_only = T, transform = transform,
...
tab1 = data$tables$tables$tab1 
if(analysis_type == 'Exploratory'){
tab1 = tab1 %>% filter(`Time Points` %in% data$inputs$timeTreatmentSelectorsTable)
}
word_tables(data = tab1, include_summ_stat = T, summary_only = F, transform = transform,
...
<br>
```{r, echo = F, error=F, warning=FALSE, fig.width=9, fig.height=6}
tab2 = data$tables$tables$tab2
if(analysis_type == 'Exploratory'){
tab2 = tab2 %>% filter(`Time Points` %in% data$inputs$timeTreatmentSelectorsTable)
}
word_tables(tab2, include_summ_stat = T, summary_only = F, transform = transform,
            footer = footer, endpoint = data$plot$endpoint)%>%
  width(x = ., width = 0.75) 
```
\newpage
...
tab3 = data$tables$tables$tab3
if(analysis_type == 'Exploratory'){
tab3 = tab3 %>% filter(`Time Points` %in% data$inputs$timeTreatmentSelectorsTable)
}
word_tables(tab3, include_summ_stat = F, summary_only = F, transform = transform,
            footer = footer, endpoint = data$plot$endpoint) %>%
  width(x = ., width = 1) 
