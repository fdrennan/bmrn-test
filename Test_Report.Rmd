...
For comparisons between doses, and doses and control there are multiple tests. To adjust for multiple comparison and preserve family wise Type I error rate, a simulation based method is used to determine the critical values and p-values based on sampling from the multivariate t distribution.[@Liu] [@berry] For comparisons between vehicle and doses. Dunnett's test will be employed which adjust p-values for comparisons between each treatment to a single control [@dunnett].  
