...
params:
  ...
  uuid: '117c9672-ddb1-4da6-993b-3fcad6043408' 
...
contrasts =  data.frame(
    ...
    `Purpose of Comparison` = c('Verify Disease Model.',
                    ...
                    'Rule out matrix effect.',
                    'If H is rejected, then show Negative Control is not as good as treatment.'),
    `Multiple Testing Procedure` = c('None','None', 'Mvt',"Dunnett's or Mvt",'Mvt','Mvt','Mvt','None','Mvt'),
  check.names = FALSE) %>%
  mutate(Comparison = paste(Group_1, 'vs.', Group_2))
...
contrasts_ft <- flextable(contrasts_display)
contrasts_ft <- footnote(x = contrasts_ft, value = as_paragraph('Mvt refers to the simulation approach using values sampled from a multivariate t distriution.'), i = 1:4 , j = 1:4, ref_symbols = "")
...
Multiple comparisons arise when a statistical analysis involves multiple simultaneous statistical tests, each of which has a potential to produce a discovery. A stated confidence level generally applies only to each test considered individually, but often it is desirable to have a confidence level for the whole family of simultaneous tests. Controlling the overall Type 1 error rate, as opposed to the Type 1 error rate of each individual test, allows us to avoid rejecting a comparison simply due to random error. For instance, if 100 comparisons were conducted each with 5\% Type I error rate, we would expect 5 results to be false positives. 
>>>>>>> 7f8d0e0b6306d13ff2e8f4cd9533cf71598aa122
...
=======
For comparisons between doses, and doses and control there are multiple tests. To adjust for multiple comparison and preserve family wise Type I error rate, a simulation based method is used to determine the critical values and p-values based on sampling from the multivariate t distribution.[@Liu] [@berry] There will be no p-value adjustments for the comparison between controls. 
>>>>>>> 7f8d0e0b6306d13ff2e8f4cd9533cf71598aa122
...
plots = vizualization(transformed_data = pdata$transformed_data, power = data$plot$data$box_cox, endpoint = data$plot$endpoint, baseline = TRUE, transformation = TRUE, ui_sel = data$plot$ui_selections)
>>>>>>> 7f8d0e0b6306d13ff2e8f4cd9533cf71598aa122
...
prism = prism_plot(data = transformed_data, tables = data$tables$tables, 
                   trt_sel = levels(transformed_data$Treatment), 
                   time_sel =  prismInputs$timePlotSelectors,
                   endpoint = data$plot$endpoint, format = 'word', 
                   cfb = data$input_data$changeFromBaseline, 
                   power = data$tables$power, 
                   num_groups = length(levels(transformed_data$Treatment)), 
